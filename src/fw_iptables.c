...
iptables_compile(const char *table, const char *chain, const t_firewall_rule *rule)
{
  ...
  if (rule->mask != NULL)
  {
    ...
    {
      snprintf((command + strlen(command)), (sizeof(command) - strlen(command)), "-m set --match-set %s dst ", rule->mask);
    }
    else
    {
...
void iptables_fw_set_roam_mac(const char *mac)
{
...
void iptables_fw_set_trusted_mac(const char *mac)
{
...
void iptables_fw_set_mac_temporary(const char *mac, int which)
{
  if (which == 0)
  { // trusted
...
void iptables_fw_save_online_clients()
{
...
int iptables_fw_init(void)
{
  ...
  if ((proxy_port = config_get_config()->proxy_port) != 0)
  {
    ...
    iptables_do_append_command(handle, "-t nat -A " CHAIN_TO_PASS " -p tcp --dport 80 -m mark --mark 0x%x0000/0xff0000 -j REDIRECT --to-port %u", FW_MARK_KNOWN,
                               proxy_port);
    iptables_do_append_command(handle, "-t nat -A " CHAIN_TO_PASS " -p tcp --dport 80 -m mark --mark 0x%x0000/0xff0000 -j REDIRECT --to-port %u", FW_MARK_PROBATION,
                               proxy_port);
  }
  ...
  iptables_do_append_command(handle, "-t filter -A " CHAIN_TO_INTERNET " -o %s -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu", ext_interface);
