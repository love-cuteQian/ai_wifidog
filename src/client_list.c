...
void offline_client_list_insert_client(t_offline_client *client)
{
...
// liudf added 20160216
void offline_client_list_destroy(t_offline_client *list)
{
...
// liudf added 20160216
void offline_client_free_node(t_offline_client *client)
{
  if (!client)
    return;
  if (client->ip)
 ...
 *
 */
void safe_client_list_delete(t_client *client)
{
  LOCK_CLIENT_LIST();
 ...
 *
 */
void offline_client_list_delete(t_offline_client *client)
{
 ...
 *
 */
void offline_client_list_remove(t_offline_client *client)
{
  t_offline_client *ptr = first_offline_client;
 ...
 *
 */
void add_online_client(const char *ip, const char *mac, json_object *client)
{
  ...
  if (!old_client)
  { // no such client in connect list
  ...
  else if (strcmp(old_client->ip, ip) != 0)
  { // The client has logined; but its ip changed
    fw_deny(old_client);
    free(old_client->ip);
